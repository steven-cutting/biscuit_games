...
system research used to inform design work across Biscuit Games and the games it serves.
It is a directory, not a second source of product requirements: the Allium specifications
decide behaviour, and the repository's own documentation decides how that behaviour is
built and checked. It was distilled from
[Mobile and game design research](research-report.md): this index is the maintained
directory, and that page is the dated argument behind it.
...
design and implementation, while remembering that the platform's named specification
guarantees can be stricter than the general standard.
...
replacement for the shared web specifications.
...
documentation, visual review, interaction testing and accessibility checks; the hub's
owning workflow is
...
decision because Biscuit Games deliberately collects no telemetry.
...
Biscuit Games currently has [no analytics or telemetry](../project/purpose-and-scope.md#what-it-deliberately-does-not-do).
...
value here is event-taxonomy thinking; it is not an authorisation to instrument the hub or
any game that follows its lead.
...
tests. It is relevant to operated live games, not the hub's current static, no-telemetry
architecture.
