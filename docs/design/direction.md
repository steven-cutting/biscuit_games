...
page for the platform's aesthetic: its identity, palette, typography, density and motion.
The character herself — her registers, her voice, and where she is allowed to be — belongs
to [The Biscuit character](character.md). It does not decide shared behaviour — the Allium
specifications under `docs/specs/` do that, and where the two ever appear to disagree, the
specifications win.
...
Biscuit Games keeps no account, runs no server and collects no telemetry. That is a real
difference from every other puzzle platform, and it is a **quiet virtue, stated once**. It
belongs on an about page. It is not the identity, it is not the marketing, and no visual
decision should be made in order to express it.
...
It is also the default. `docs/specs/appearance.allium` starts the theme dark rather than
following the device, so a first visit lands on near-black whatever the device prefers, with
light and system one choice away wherever the product that owns the controls puts them; the
markup ships the same answer so the first paint agrees.

`src/app.css` is still structured light-first, because bare `:root` has to hold some palette
and the light one is the fallback, but the figures were chosen on the near-black page and
the light values answer to them. Colour decisions are made for dark and then answered for
light, not the reverse. Nothing about the four-way theme and high-contrast structure
changes; only which of the four is designed first.
...
Motion is also a setting. `docs/specs/appearance.allium` declares animations as one and
makes a device's request for reduced motion win over it, whatever the setting says; what
happens then is stated under [Biscuit](#biscuit) rather than left to each component to
decide.
...
host rather than its logo. She owns the place: the dark room, the tight grid and the instant
response are her taste, and the one break the operating rule allows on every layer is her.

She is rationed. She belongs at the boundaries — opening, the end of a game, sign-off, and
ambient states — and is absent while a player is thinking, so she never competes with a
board for attention and never obscures state. Her face is fixed, learned by heart, and it
breaks so seldom that the break is itself the payoff. Rarely means rarely: a break that
shows up often enough to be expected has already stopped being one. The registers, the
split voice, the first pose set and the places she is not allowed to go are all owned by
[The Biscuit character](character.md).

When animations are off or reduced motion is requested, **she reduces to the mark** — only
the icon-mark and header remain. That is a decision, not an oversight, and it has a cost
worth naming: those players lose the character reward entirely and are carried by craft
alone. That is consistent with craft being what carries a first visit anyway, and it is
preferable to a half-animated compromise. The obligations in
[Accessibility](../explanation/accessibility.md) are unaffected — Biscuit is decorative,
carries no state, and never conveys a result.
