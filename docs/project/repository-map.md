...
│   │   ├── components/  PascalCase Svelte components, plus icons.ts and layouts.ts
│   │   ├── domain/      The appearance derivations, the typing rule, and the types
│   │   ├── ports/       The preferences and keys ports: interface, adapter, fake
│   │   ├── config.ts    The six figures the three modules declare
...
| `src/lib/domain/` | Pure functions: the three derivations `appearance.allium`'s `Appearance` surface states, and the rule `operation.allium`'s `TypedInput` states for what a bare key press means. With them the `ThemeChoice` enumeration and the `Mark` a play surface paints. |
| `src/lib/ports/` | Two: what the reader asked their operating system for, read through `matchMedia` and watched; and the device's keyboard, subscribed to for as long as a surface claims it. Each is an interface, an adapter taking its host as a defaulted argument, and the fake a test injects. |
...
- No arrangement of a play surface. The cell and the key are here since
  [decision 0016](../decisions/0016-the-play-surface-is-the-platforms.md), but a board, a
  rack and a grid are not: an arrangement encodes a rule, so it stays in the game that
  renders it. The ledger in
  [Port a design system component](../how-to/port-a-design-system-component.md) says which
  rows those are, and says why each was refused rather than leaving it to omission.
