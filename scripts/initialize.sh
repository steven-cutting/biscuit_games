...
# the tooling package that pyproject.toml pins, landing in the gitignored
# .tools/bin. `just check-specs` and `just analyse-specs` run it, and both the
# hook gate and `just check` run those, so a worktree without it cannot reach a
# green gate.
