 ...
 * Appearance globals.
 *
 * `appearance.allium`'s Appearance surface derives `dark_active` and
 * `animations_active` from the stored settings and from the device. The toolbar
 * below makes those inputs adjustable, which is what lets a component be
 * inspected in all four combinations of theme and high contrast — the bar
 * `EveryCombinationMeetsTheLegibilityFloor` sets.
 *
...
const REDUCED_MOTION_STYLE_ID = 'biscuit-simulated-reduced-motion';
