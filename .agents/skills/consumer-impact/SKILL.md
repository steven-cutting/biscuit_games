...
7. Write each finding into the handover page as a checklist item naming the file to change and the value to change it to, and say which release carries it. Do not edit another repository: that needs explicit authorization for each action, and this repository records rather than acts.
