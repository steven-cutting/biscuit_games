...
6. Look at the change in the workshop, driving theme, high contrast, animations and simulated reduced motion from the toolbar — there is no token specimen story here, so a component story is where a token is seen. Then request `/chromatic` on the pull request: a token change is a visual change, and that review is the design review.
