...
/*
 * appearance.allium — the `Appearance` surface. It applies to the hub and to
 * every game wearing the design system, and it is where the reader's settings
 * and the device's preferences negotiate.
 */
describe('darkActive', () => {
  ...
  it('stops consulting the device once the reader picks a side', () => {
...
describe('animationsActive', () => {
  ...
  it('gives no motion to a reader whose system asked for less, whatever the setting says', () => {
...
describe('highContrastActive', () => {
  ...
  it('applies when the reader asks and the device is silent', () => {
    expect(highContrastActive(true, false)).toBe(true);
  });

  it('does not apply when neither asks', () => {
