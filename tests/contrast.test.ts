/*
 * `docs/specs/appearance.allium` — Appearance.@guarantee
 ...
 *
 * The pairs are the stylesheet's, measured as declarations. The hub renders
 * few of them itself — `Button`, `IconButton`, `HeaderBar`'s chip and `Notice`
 * are the platform controls — but the stylesheet is what every game wears, and
 * a token that fails here fails on a board this repository has never seen. The
 * result tokens are measured for that reason: a game's scored key is this
 * palette, so its legibility is decided here. What the guarantee leaves to the
 * game — how far one of its states sits from another — is measured in the
 * game's own gate against figures its own specification states, which is why
 * no separation appears below.
 *
 * Axe cannot stand in for this, and the two gates fail differently. Axe checks
 * what it can attribute to painted text in one rendered story under one
 * combination, so it never sees a hairline boundary, a decorative marker or a
 * token nothing in the workshop happens to paint; this measures the palette.
 *
 ...
 * reader cannot operate to none of these figures, exactly as WCAG 2.2 exempts
 * an inactive component from 1.4.3 and 1.4.11 — so `--text-disabled` and the
 * disabled button's `--rule` border are measured nowhere here, and adding them
 * would assert a floor the specification does not state. The half of that
 * guarantee which does bind is not a ratio at all: a dimmed control still
 * reports its state and keeps its words, which `tests/primitives.test.ts`
 * holds where the controls are rendered.
 *
...
// The two inks a scored result is painted in. Absent is deliberately not
// here: it has no ink of its own, and the two pairs it answers — its letter
// against the untried letter, its border against the page — are stated
// individually below.
...
describe('EveryCombinationMeetsTheLegibilityFloor', () => {
  for (const combination of COMBINATIONS) {
    describe(combination.name, () => {
       ...
       * honest bar — so a future palette pinned by a fill may re-derive that
       * pair at 3.0 rather than quietly weakening this file.
       */
      it('paints legible text on every ground a control is drawn on', () => {
       ...
       * hairline on the page — which is why no control may borrow `--rule` or
       * `--rule-strong` instead. A hue result's border is its ink, and
       * absent's is the one border it draws.
       */
      ...
      /*
       * `--surface` is the dialog ground, and the one ground a result had not
       * otherwise met: a game's board is on the page, its scored key on the
       * raised ground. A game that shows its marks inside a dialog — every game
       * explains itself somewhere — puts them here, so each pair below is one
       * the eye can actually meet.
       *
       ...
       * reason. The border is on the surface in every theme, and it is the
       * shape that carries the result, so it answers to the non-text bar
       * throughout.
       */
      it('paints a result legibly on a dialog surface', () => {
...
describe('the palette these floors are met by', () => {
   ...
   * because it is what lets a game's separation figures be reachable at all.
   * The untried key does not sit near the page: it sits on it, which is what
   ...
   * would have nowhere to stand, and a game's state separations would be paid
   * twice.
   */
   ...
   * measured above in every palette. A measured token nothing renders is a
   * figure that cannot regress where anyone would see it, though — which is
   * how this pair once reached the branch defined, tested and unspent — so
   * what is left to state is that the stylesheet still spends it. Read as text
   * for the reason the drift check below records: a rule the engine declined
   * to parse becomes an empty declaration list, and an empty block agrees with
   * anything.
   */
...
describe('the two ways the dark theme is reached', () => {
   ...
   * declared would leak into a device-dark reader who asked for high
